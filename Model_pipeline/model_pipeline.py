...
data = pd.read_csv(
    r"C:\Users\ADMIN\Documents\OWN_Projects\Predictive-Risk-Management-System-for-Non-Performing-Loans-in-Kenyan-Banks\Notebook\cleaned_preprocessed_loans_data.csv"
)
...
# ============================
# Grade 3: these are helpers to draw and save plots for each model.
...
def plot_and_save_roc(y_true, y_scores, title, filepath):
    # Grade 3: draw ROC curve and show AUC number, then save the picture.
...
def plot_and_save_pr(y_true, y_scores, title, filepath):
    # Grade 3: draw precision-recall curve and show PR AUC, then save the picture.
...
def run_grid_search(name, estimator, param_grid):
    ...
    try:
        ...
        # Grade 3: some models may not have predict_proba; fallback to decision_function then scale.
        try:
            val_scores = gs.decision_function(X_val)
            # convert scores to 0-1 via min-max (simple)
            y_val_proba = (val_scores - val_scores.min()) / (val_scores.max() - val_scores.min() + 1e-8)
        except Exception:
            # Grade 3: last fallback, use predictions as 0/1 probabilities
    ...
    y_test_pred_local = best_model.predict(X_test)

    try:
        y_test_proba_local = best_model.predict_proba(X_test)[:, 1]
    except Exception:
        try:
    ...
    plot_and_save_pr(y_test, y_test_proba_local, f"{name} - Test PR Curve", f"plots/{name}_test_pr.png")
